...
/* ── SHA-256 (compact portable implementation) ───────────── */
/*
 * Self-contained SHA-256. No dependency on OpenSSL or kernel
 * crypto subsystem. Used only for HMAC key derivation.
...
typedef struct { u8 data[64]; u32 state[8]; u64 bitlen; u32 datalen; } sha256_ctx_t;
...
static void sha256_final(sha256_ctx_t *ctx, u8 *hash) {
    ...
    ctx->data[63]=(u8)ctx->bitlen;       ctx->data[62]=(u8)(ctx->bitlen>>8);
    ctx->data[61]=(u8)(ctx->bitlen>>16); ctx->data[60]=(u8)(ctx->bitlen>>24);
    ctx->data[59]=(u8)(ctx->bitlen>>32); ctx->data[58]=(u8)(ctx->bitlen>>40);
    ctx->data[57]=(u8)(ctx->bitlen>>48); ctx->data[56]=(u8)(ctx->bitlen>>56);
    sha256_transform(ctx,ctx->data);
    for (i=0;i<4;i++) {
        hash[i]   =(u8)(ctx->state[0]>>(24-i*8));
        hash[i+4] =(u8)(ctx->state[1]>>(24-i*8));
        hash[i+8] =(u8)(ctx->state[2]>>(24-i*8));
        hash[i+12]=(u8)(ctx->state[3]>>(24-i*8));
        hash[i+16]=(u8)(ctx->state[4]>>(24-i*8));
        hash[i+20]=(u8)(ctx->state[5]>>(24-i*8));
        hash[i+24]=(u8)(ctx->state[6]>>(24-i*8));
        hash[i+28]=(u8)(ctx->state[7]>>(24-i*8));
    }
}

void sha256_impl(const u8 *in, size_t len, u8 out[32]) {
...
/* ── HMAC-SHA256 ─────────────────────────────────────────── */
void hmac_sha256_impl(const u8 *key, size_t klen,
...
int cc_kernel_attest_process(pid_t pid, u8 *token, u32 *size)
{
    ...
    *(u64*)(plain+16) = t->neri.ram_granted_pages;
    ...
    sha256_impl((const u8*)t->name, strnlen(t->name, 31), plain+32);
    ...
    *(u64*)(plain+64) = g_boot_timestamp;
    ...
    /* AES-256-GCM encrypt */
    aes256gcm_encrypt(g_attest_key, nonce, plain, ATTEST_PLAIN_LEN,
...
int cc_kernel_verify_attestation(const u8 *token, u32 size, pid_t expected_pid)
{
    ...
    int rc = aes256gcm_decrypt(g_attest_key, nonce, ciphertext,
...
int cc_kernel_seal_memory(virt_addr_t va, u64 pages, void *out, u32 *size)
{
    ...
    u8 aad[8]; *(u64*)aad = va;
    aes256gcm_encrypt(g_secure_alloc_key, nonce,
...
ssize_t cc_ipc_send(int fd, const void *buf, size_t len)
{
    ...
    memcpy(dst, nonce, 12);
    *(u32*)(dst+12) = (u32)len;
    aes256gcm_encrypt(ch->session_key, nonce,
...
ssize_t cc_ipc_recv(int fd, void *buf, size_t len)
{
    ...
    int rc = aes256gcm_decrypt(ch->session_key, nonce, ct, plen,
                               tag, nonce, 12, (u8*)buf);
    *head += 12 + 4 + plen + 16;
...
int cc_kernel_checkpoint(struct task *t, void *out, u32 *size)
{
    ...
    *(u64*)(plain+64)    = t->cpu_time_ns;
    ...
    aes256gcm_encrypt(g_attest_key, nonce, plain, 256, aad, 4, ct, tag);
